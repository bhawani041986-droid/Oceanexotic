...
foreach ($catches as $catch) {
    ...
    $sql = "INSERT INTO todays_catch (id, product_id, seller_id, catch_date, harbor_node, quantity_kg, remaining_kg, price_per_kg, freshness_timestamp, expires_at, batch_label, status) 
            VALUES ('$id', '$catch[0]', '$catch[1]', '$today', '$catch[2]', $catch[3], $catch[4], $catch[5], '$now', '$expiry', '$catch[6]', 'ACTIVE')";
    $c->query($sql);
}
...
foreach ($cutOptions as $opt) {
    $cut_id = "CUT-" . $opt[0] . "-" . $opt[1] . "-" . microtime(true);
    $sql = "INSERT INTO product_cut_options (id, product_id, cut_type, price_modifier_percent, price_flat_add, is_available, stock_kg, sort_order) 
            VALUES ('$cut_id', '$opt[0]', '$opt[1]', $opt[2], $opt[3], $opt[4], $opt[5], $opt[6])";
    $c->query($sql);
}
