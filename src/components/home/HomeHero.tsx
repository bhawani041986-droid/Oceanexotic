    ...
    <section className="relative min-h-[85vh] lg:min-h-screen flex items-center justify-center pt-24 overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-20 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} className="space-y-10 text-center lg:text-left">
          <Badge variant="glass" className="px-6 py-2 text-[10px] font-black tracking-[0.4em] text-primary">MARITIME SOVEREIGNTY</Badge>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
             ...
             <Button variant="outline" className="h-16 px-12 rounded-full text-[12px] font-black uppercase tracking-widest border-[var(--foreground)]/10" onClick={() => router.push('/customer/products')}>EXPLORE HARVESTS</Button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="hidden lg:block relative">
           ...
           <Card className="p-10 bg-bg-card/40 backdrop-blur-3xl border-[var(--foreground)]/10 rounded-[64px] shadow-2xl space-y-8 relative z-10 hover:translate-y-[-10px] transition-transform duration-700">
              ...
              <div className="flex justify-between items-end"><h3 className="text-4xl font-black text-[var(--foreground)] uppercase italic">Saku Grade</h3><p className="text-3xl font-black text-primary italic">₹2,450</p></div>
              <Button className="w-full h-16 rounded-3xl shadow-glow-purple text-[11px] font-black uppercase">COMMISSION HARVEST</Button>
           </Card>
