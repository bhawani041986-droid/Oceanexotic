...
export const AdminMobileTopNav = ({ title }: { title: string }) => {
  ...
  const navItems = [
    ...
    { label: "Support Chat", icon: <MessageSquare className="w-5 h-5" />, href: "/admin/support", color: "#A855F7" },
    { label: "Fraud & Security", icon: <ShieldAlert className="w-5 h-5" />, href: "/admin/security", color: "#EF4444" },
    { label: "Yield Settlements", icon: <Banknote className="w-5 h-5" />, href: "/admin/withdrawals", color: "#14B8A6" },
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 h-[64px] bg-bg-primary/60 backdrop-blur-2xl border-b border-[var(--foreground)]/5 z-[100] px-4 flex items-center justify-between lg:hidden">
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setIsMenuOpen(true)}
            className="w-10 h-10 flex items-center justify-center text-text-primary hover:bg-[var(--foreground)]/5 rounded-xl transition-all active:scale-90"
          >
        ...
        <div className="flex items-center gap-1">
          <button 
            ...
            className="w-10 h-10 flex items-center justify-center text-text-secondary hover:text-text-primary transition-colors active:scale-90"
          >
            <Search className="w-5 h-5" />
          </button>
          <button className="w-10 h-10 flex items-center justify-center text-text-secondary hover:text-text-primary relative active:scale-90">
            <Bell className="w-5 h-5" />
            <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-danger rounded-full border-2 border-bg-primary" />
          </button>
          <div className="w-8 h-8 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center ml-2 overflow-hidden shadow-glow-purple">
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link 
                      ...
                      onClick={() => setIsMenuOpen(false)}
                      className={cn(
                        "flex items-center gap-2.5 px-3 py-2 text-[9px] font-black uppercase tracking-[0.2em] transition-all group relative overflow-hidden",
                        isActive 
...
export const AdminMobileBottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-6 left-4 right-4 h-[72px] bg-bg-card/80 backdrop-blur-3xl border border-[var(--foreground)]/10 rounded-[28px] shadow-2xl z-[100] flex items-center justify-around px-2 lg:hidden">
      {navItems.map((item) => {
        const isActive = pathname === item.href;
        return (
          <Link 
