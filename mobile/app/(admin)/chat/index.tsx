...
export default function ChatInboxScreen() {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <Pressable
      onPress={() => router.push({ pathname: "/(admin)/chat/[id]", params: { id: item.id, name: item.other_party_name, role: item.other_party_role } } as never)}
    ...
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      ...
      <NewChatModal 
        ...
        onChatCreated={(convId) => {
          ...
          router.push({ pathname: "/(admin)/chat/[id]", params: { id: convId.toString(), name: "New Chat", role: "admin" } } as never);
        }}
      />
    </View>
  );
