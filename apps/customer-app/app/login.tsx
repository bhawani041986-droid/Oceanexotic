  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      if (result.success && result.user && result.token) {
        ...
        if (
          user.role &&
          !["customer", "agent", "admin", "seller"].includes(user.role)
        ) {
          ...
          <View className="mx-auto w-full max-w-[400px] mt-auto mb-auto">
            ...
            <View className="gap-4 w-full">
              ...
              <View className="flex-row items-center justify-between my-2">
                ...
                <Text className="text-[9px] font-black uppercase text-slate-500 tracking-widest mx-4">
                  {t("or_use_email") || "or use email"}
                </Text>
                <View className="flex-1 h-[1px] bg-white/10" />
              </View>
              ...
              <View className="bg-white/5 border border-white/10 p-6 space-y-4">
                ...
                {submitError ? (
                  <Text className="text-center text-[10px] font-bold text-red-400">
                    {submitError}
                  </Text>
                ) : null}
