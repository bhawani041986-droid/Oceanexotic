...
export default function ProductsScreen() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    if (slug === "seawater" || slug === "reef" || slug === "snapper")
      return "Seawater Fish";
    if (slug === "freshwater" || slug === "river" || slug === "mackerel")
      return "Freshwater Fish";
    if (slug === "prawns" || slug === "shrimp") return "Prawns & Shrimps";
    if (slug === "crustaceans" || slug === "crab" || slug === "lobster")
      return "Crabs & Lobsters";
    if (slug === "fillets" || slug === "steaks" || slug === "kingfish")
      return "Steaks & Fillets";
    if (slug === "exotic" || slug === "premium") return "Exotic Catch";
    if (slug === "ready-to-cook" || slug === "ready") return "Ready to Cook";
    if (slug === "dry-fish" || slug === "dry" || slug === "pomfret")
      return "Coastal Dry Fish";
  ...
  const categorySections = useMemo(() => {
    return TABS.filter((t) => t !== "All Seafood" && t !== "Ready to Cook")
      .map((tab) => ({
        title: tab,
  ...
  const isLoading =
    catalog.isLoading || (searchQuery.trim() ? search.isLoading : false);
  ...
  const dynamicTickerItems = useMemo(() => {
    const couponTicker = activeCoupons.map(
      (c: any) =>
    ...
    () =>
      displayList.filter(
        (p) =>
          ...
          /marinate|grill|fry|masala|ready|spice/i.test(
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      ...
      >
        <SectionTitle
          title={t("product_catalog") || "Product Catalog"}
          subtitle={
            t("catalog_subtitle") || "Premium Seafood Discovery • Fast Delivery"
          }
        />
            ...
            <View className="flex-row items-center justify-between gap-3">
              <View className="flex-1 gap-1">
                <View className="self-start px-2 py-0.5 bg-primary">
                  <Text className="text-[7px] font-black uppercase tracking-widest text-black">
                    {t("exclusive_offer")}
                  </Text>
                </View>
                <Text
                ...
                >
                  {t("apply_code_at_checkout")}
                  {activeCoupons[0].min_purchase > 0 &&
                    ` ${t("min_order")}: ₹${activeCoupons[0].min_purchase}.`}
                </Text>
              </View>

              <View className="items-center bg-slate-900 border border-white/10 p-2.5 min-w-[100px]">
                <Text className="text-[6px] font-black uppercase tracking-widest text-slate-400 italic">
                  {t("promo_code")}
                </Text>
                <Text className="text-xs font-black tracking-[0.15em] mt-1 text-primary">
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                >
                  {t(
                    tab.toLowerCase().replace(/ & /g, "_").replace(/ /g, "_"),
                  ) || tab}
                </Text>
              </Pressable>
            );
        ...
        {isLoading ? (
          ...
          >
            <Text
              ...
              style={{ color: colors.textMuted }}
            >
              {t("no_harvest") || "No product in this sector"}
            </Text>
            <Pressable onPress={() => router.replace("/home")} className="mt-4">
              <Text
                ...
                style={{ color: colors.primary }}
              >
                {t("return_home") || "Return to Harbor Home"}
              </Text>
            </Pressable>
          </View>
        )}
