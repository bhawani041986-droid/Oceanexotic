  ...
  return (
    ...
    >
      ...
      <Svg width="100%" height="100%" style={StyleSheet.absoluteFill}>
        ...
        />
        <Path
          d="M0,250 Q120,200 240,250 T480,250 T720,250"
          stroke={colors.primary}
          strokeWidth="0.8"
          opacity="0.05"
          fill="none"
        />
        <Path
          d="M0,450 Q80,480 160,450 T320,450 T480,450 T640,450 T800,450"
          stroke={colors.primary}
          strokeWidth="1.2"
          opacity="0.06"
          fill="none"
...
export default function RecipeDetailsScreen() {
    ...
    if (foundLocal) {
      return {
        ...
        metadata: JSON.stringify({
          difficulty: foundLocal.difficulty,
          time: foundLocal.time,
        }),
  ...
  const meta = useMemo(() => {
    ...
    try {
      return typeof recipe.metadata === "string"
        ...
        <View className="px-5 pt-6 space-y-8">
          ...
          <View className="space-y-4">
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-sm font-black uppercase text-foreground tracking-widest">
                {t("efficiency_telemetry") || "Efficiency Tracking"}
              </Text>
            </View>
            ...
            >
              ...
              <View className="flex-row justify-between items-center mb-4">
                <View>
                  <Text className="text-[10px] font-black uppercase text-slate-400">
                    {t("omega3_concentration") || "Omega-3 Concentration"}
                  </Text>
                  <Text className="text-lg font-black text-white italic mt-0.5">
              ...
              <View className="space-y-2">
                <View className="flex-row justify-between text-[9px] font-black uppercase tracking-wider text-slate-400">
                  <Text>
                    {t("bio_absorption_index") || "Bio-absorption Index"}
                  </Text>
              ...
              <View className="flex-row justify-between mt-5 pt-4 border-t border-white/5">
                <View className="items-center flex-1">
                  ...
                  <Text className="text-sm font-black text-emerald-400 mt-0.5">
                    {t("protein_34g") || "34g"}
                  </Text>
                </View>
                <View className="items-center flex-1 border-x border-white/5">
                  ...
                  <Text className="text-sm font-black text-cyan-400 mt-0.5">
                    {t("kcal_320") || "320 kcal"}
                  </Text>
                </View>
                <View className="items-center flex-1">
                  <Text className="text-[9px] font-black text-slate-500 uppercase">
                    {t("prep_level") || "Prep Level"}
                  </Text>
                  <Text className="text-sm font-black text-amber-400 mt-0.5">
                    {t("medium") || "Medium"}
                  </Text>
                </View>
              </View>
          ...
          <View className="space-y-4">
            <View className="flex-row items-center gap-2">
              ...
              />
              <Text className="text-sm font-black uppercase text-foreground tracking-widest">
                {t("required_elements") || "Required Elements"}
              </Text>
            </View>

            <LinearGradient
              colors={["rgba(30, 41, 59, 0.4)", "rgba(15, 23, 42, 0.65)"]}
              className="p-5 border relative overflow-hidden space-y-3.5"
              style={{ borderColor: "rgba(255, 255, 255, 0.08)" }}
            >
              ...
              {(ingredients as string[]).map((ing: string, i: number) => (
                <View key={i} className="flex-row items-center gap-3.5">
                  ...
                  <Text className="text-sm text-foreground/80 flex-1 leading-relaxed font-medium">
                    {ing}
                  </Text>
                </View>
              ))}
            ...
            <View className="space-y-4">
              {(steps as string[]).map((step: string, i: number) => (
                ...
                >
                  ...
                  <Text className="text-sm text-foreground/90 flex-1 leading-relaxed pt-0.5 font-medium">
                    {step}
                  </Text>
                </LinearGradient>
              ))}
