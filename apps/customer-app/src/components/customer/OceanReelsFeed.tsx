...
export function OceanReelsFeed({
  ...
  const handleAddToCart = (product: any) => {
    ...
    toast(
      `${product.name} ${t("added_from_reels") || "added from Ocean Reels!"}`,
      "success",
    );
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            <View className="absolute left-2 top-2 bg-red-500/90 px-2 py-0.5 z-20">
            ...
            {isActive && (
              <Pressable
                ...
                className="absolute top-2 right-2 p-1.5 bg-black/50 z-20"
              >
            ...
            {!isActive && (
              <View className="absolute inset-0 items-center justify-center pointer-events-none">
                <View className="w-8 h-8 items-center justify-center bg-white/20 border border-white/40">
                  <MaterialCommunityIcons
                    name="play"
                    size={16}
                    color="white"
          ...
          <View className="gap-2 p-3" style={{ minHeight: 90 }}>
            ...
            {product && (
              <View className="flex-row items-center justify-between mt-auto">
                <Text
                  className="text-sm font-black italic text-foreground"
                  style={{ color: colors.text }}
                >
                  ₹{product.price}
                </Text>
  ...
  if (variant === "banner") {
    const vid = videoId
      ...
      : videos.find((v) => v.description === "banner") || videos[0];
    if (!vid) return null;
    const product = allProducts?.find((p) => p.id === vid.product_id);
    const isActive = activeVideoId === vid.id;
    ...
    return (
      <Pressable
        onPress={() => setActiveVideoId(isActive ? null : vid.id)}
        className="w-full my-4 flex-row overflow-hidden border"
        style={{
          height: 144,
      ...
      >
        ...
        >
          ...
          <View className="absolute left-1.5 top-1.5 bg-red-500/95 px-1 py-0.5 z-20">
          ...
          {isActive && (
            <Pressable
              ...
              className="absolute top-1.5 right-1.5 p-1 bg-black/60 z-20"
            >
              <MaterialCommunityIcons
                name={isMuted ? "volume-mute" : "volume-high"}
                size={10}
          ...
          {!isActive && (
            <View className="absolute inset-0 items-center justify-center pointer-events-none">
              <View className="w-6 h-6 items-center justify-center bg-white/20 border border-white/40">
                <MaterialCommunityIcons
                  name="play"
                  size={12}
                  color="white"
        ...
        >
          <View className="space-y-1">
            ...
            >
              {vid.description ||
                "Fresh premium catch sourced directly from local harbors. 100% traceable cold-chain delivery."}
            </Text>
          </View>

          <View
            className="flex-row items-center justify-between pt-1 border-t"
  ...
  if (variant === "pip") {
    ...
    return (
      ...
      >
        {isPipOpen && (
          <View
            className="border bg-black shadow-2xl mb-3 overflow-hidden"
          ...
          >
            ...
            <Pressable
              ...
              className="absolute top-2 left-2 p-1.5 bg-black/40"
            >
              <MaterialCommunityIcons name="close" size={14} color="white" />
            </Pressable>
            <Pressable
              onPress={() => setIsMuted(!isMuted)}
              className="absolute top-2 right-2 p-1.5 bg-black/40"
            >
              <MaterialCommunityIcons
                name={isMuted ? "volume-mute" : "volume-high"}
                size={14}
            ...
            <View className="absolute bottom-0 left-0 right-0 p-2">
              <Text className="text-white font-bold text-[9px] truncate mb-0.5">
                {vid.title}
              </Text>
              {product && (
                <View className="flex-row items-center justify-between">
                  ...
                  <Pressable
                    ...
                    className="px-2 py-0.5 bg-primary"
                    style={{ backgroundColor: colors.primary }}
                  >
        ...
        <Pressable
          ...
          className="w-16 h-16 border-2 overflow-hidden shadow-lg items-center justify-center p-0.5 bg-black"
        ...
        >
          <View className="w-full h-full overflow-hidden relative">
            ...
            <View className="absolute inset-0 bg-black/20 items-center justify-center">
              <View
                className="w-6 h-6 items-center justify-center bg-primary/95"
                style={{ backgroundColor: colors.primary }}
              >
                <MaterialCommunityIcons
                  name="play"
  ...
  return (
    <View
      className="w-full py-6 border-t border-b my-4"
      style={{ borderColor: `${colors.border}20`, backgroundColor: colors.bg }}
    >
        ...
        {videos.map((vid) => {
          ...
          return (
            <Pressable
              key={vid.id}
              onPress={() => setActiveVideoId(isActive ? null : vid.id)}
              className="relative overflow-hidden bg-black shadow-md border"
              style={{
                width: 120,
                height: 200,
                ...
                {product && (
                  <View className="flex-row items-center justify-between mt-1">
                    ...
                    <Pressable
                      ...
                      className="h-6 w-6 items-center justify-center shadow-lg"
                      style={{ backgroundColor: colors.primary }}
                    >
                      <MaterialCommunityIcons
                        name="cart-plus"
