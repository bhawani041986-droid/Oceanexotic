...
export function AndamanMaritimeTelemetry({ territories = [] }: TelemetryProps) {
  ...
  const [lngCenter, setLngCenter] = useState(92.75);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const activeNodes = (territories || []).filter(
        (t) => t.status === "ACTIVE" && t.coordinates,
      );

      const mainHub = activeNodes.find((t) =>
        t.name.toLowerCase().includes("port blair"),
      );
      let hubPos: any = null;
      if (mainHub) {
        const hp = String(mainHub.coordinates)
          .split(",")
      ...
      activeNodes.forEach((t, i) => {
        const raw = String(t.coordinates)
          .split(",")
          .map((s) => parseFloat(s.trim()));
 ...
 <div class="absolute w-6 h-6 border border-[var(--foreground)]/10 animate-ping" style="border-color: ${color}22"></div>
 <div class="w-2.5 h-2.5 border border-white shadow-[0_0_8px_${color}]" style="background-color: ${color}"></div>
 </div>
 ...
 <div class="absolute bottom-[10px] left-1/2 -translate-x-1/2 bg-black/95 border-b-2 px-2.5 py-1 flex flex-col items-center shadow-[0_0_20px_rgba(0,0,0,0.6)] whitespace-nowrap" 
 ...
 <div class="w-1 h-1 bg-emerald-500 animate-pulse" />
 </div>
 ...
 <span class="text-[5px] font-mono text-[${color}] opacity-80">${t.coordinates}</span>
  ...
  const mappedNodes = activeNodes.map((node, index) => {
    const parts = String(node.coordinates)
      .split(",")
    ...
    if (parts.length >= 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
      ...
      y = Math.max(
        20,
        Math.min(radarSize * 0.75 - 20, yPct * (radarSize * 0.75)),
      );
    }
  ...
  const hubNode =
    mappedNodes.find((item) =>
      item.node.name.toLowerCase().includes("port blair"),
    ) || mappedNodes[0];
  ...
  if (Platform.OS === "web") {
    return (
      <View className="px-4 py-6 border-y border-white/5 bg-secondary/20">
        <View className="mb-4">
          <Text className="text-xl font-black italic uppercase text-foreground">
            {translate("live_delivery_map") || "Live Delivery Map"}
          </Text>
          <Text
            className="text-[9px] font-black uppercase tracking-[0.3em]"
            style={{ color: colors.primary }}
          >
            {translate("real_time_delivery_hub_mapping") ||
          ...
          >
            <View className="flex-1 flex-row items-center gap-3 p-3">
              ...
              <View className="flex-1">
                <Text className="text-[8px] font-black text-rose-500 uppercase tracking-[0.1em]">
                  {translate("done") || "DONE"}
                </Text>
                <Text className="text-xs font-black text-foreground uppercase italic">
                  {translate("fleet_auth") || "Delivery Auth"}
                </Text>
              </View>
            </View>
          </ChamferedBox>
 ...
 `,
          ...
          >
            <Text
              ...
              style={{ color: colors.primary }}
            >
              {translate("sector") || "Sector"}: ALPHA-6
            </Text>
          </View>
    ...
    <View className="px-4 py-6 border-y border-white/5 bg-secondary/20">
      ...
      >
        ...
        >
          ...
          <RNImage
            source={{
              uri: `https://static-maps.yandex.ru/1.x/?ll=${lngCenter},${latCenter}&z=${zoom}&l=map&size=600,450`,
            }}
            style={{
              width: "100%",
              height: "100%",
          ...
          <Svg width="100%" height="100%" className="absolute">
            ...
            <Line
              x1="0"
              y1={radarSize * 0.75 * 0.2}
              x2={radarSize}
              y2={radarSize * 0.75 * 0.2}
              stroke={colors.primary + "0D"}
              strokeWidth="1"
            />
            <Line
              x1="0"
              y1={radarSize * 0.75 * 0.4}
              x2={radarSize}
              y2={radarSize * 0.75 * 0.4}
              stroke={colors.primary + "0D"}
              strokeWidth="1"
            />
            <Line
              x1="0"
              y1={radarSize * 0.75 * 0.6}
              x2={radarSize}
              y2={radarSize * 0.75 * 0.6}
              stroke={colors.primary + "0D"}
              strokeWidth="1"
            />
            <Line
              x1="0"
              y1={radarSize * 0.75 * 0.8}
              x2={radarSize}
              y2={radarSize * 0.75 * 0.8}
              stroke={colors.primary + "0D"}
              strokeWidth="1"
            />
            <Line
          ...
          <View className="absolute inset-0">
            {mappedNodes.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <Text
                        ...
                        className="text-[5.5px] font-mono opacity-80 text-center"
                      >
              ...
              <View className="absolute inset-0 items-center justify-center">
                <Text
                  style={{ color: colors.primary }}
                  className="text-[18px] font-black tracking-widest animate-pulse"
                >
                  {translate("radar_secure") || "RADAR SECURE"}
                </Text>
                <Text className="text-muted-foreground text-[8px] uppercase tracking-widest mt-1">
                  {translate("andaman_fresh_catch_market") ||
                    "Andaman Fresh Catch Market"}
                </Text>
              </View>
            )}
          </View>
