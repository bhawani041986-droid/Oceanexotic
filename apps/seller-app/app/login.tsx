  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      if (result.success && result.user && result.token) {
        ...
        if (user.role !== "seller") {
          const message = "Access Denied: Only Sellers can access this dashboard.";
          toast(message, "error");
        ...
        <ScrollView contentContainerClassName="flex-grow px-6 py-12" keyboardShouldPersistTaps="handled" bounces={false}>
          <View className="mx-auto w-full max-w-[400px] mt-auto mb-auto">
            ...
            <View className="gap-4 w-full">
              <View className="rounded-3xl bg-white/5 border border-emerald-400/20 p-6 space-y-4">
                <Controller
                  control={control}
                  name="email"
                  render={({ field: { onChange, onBlur, value } }) => (
                    <Input
                      ...
                      onBlur={onBlur}
                      placeholder="Vendor Fleet Identity (Email)"
